...
typedef struct {
  float ax, ay, az, gx, gy, gz, mx, my, mz, tv; //possibly double?
  double lat, lng;
  int hour, min, sec, csec;
} movementType;
movementType data[19];//[9];
...
void magPoll() {
   mag.getEvent(&event);
   data[arrayPtr].mx = event.magnetic.x;
   data[arrayPtr].my = event.magnetic.y;
   data[arrayPtr].mz = event.magnetic.z;
